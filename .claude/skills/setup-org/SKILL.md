...
| `ai-agent-organization/organization/operations/note-creator-NOTE_CREATOR/knowledge/knowhow/creator-achievements.md` | 権威性・実績まとめ **【既存があれば改変・なければ新規作成】** | Step 4 |
...
Step 2で特定した関心セグメントに対応するペルソナファイルを用意します。
...
**Step 2のセグメントへの対応方針（優先順位順）：**

1. **スケルトンの数（4個）以内のセグメント数の場合 → スケルトンを直接改変する**
   - 各スケルトンファイルの内容をStep 2のセグメントに合わせて書き換える
...
| ファイルパス | 反映内容 | 参照ステップ |
|------------|---------|------------|
| `ai-agent-organization/organization/operations/gunshi-GUNSHI/knowledge/knowhow/account-design.md` | Step 1〜6 の情報をまとめた戦略設計書 **【既存があれば改変・なければ新規作成】** | Step 1〜6 全体 |
| `ai-agent-organization/organization/operations/gunshi-GUNSHI/knowledge/knowhow/master-strategy.md` | コンテンツ戦略・KPI・商品ラインの全体設計（未定項目は `[未定]` プレースホルダーで残す） **【既存があれば改変・なければ新規作成】** | Step 2・5・6 |
