# AIっぽくない文章に仕上げるリライト用マスタープロンプト

記事完成後の最終仕上げ工程で使用するリライト用プロンプト。
NOTE_CREATORやARTICLE_STYLISTが最終チェック・リライト時に参照する。
...
## 注意事項（note記事での運用）
- 見出し（##）のMarkdown、引用(>)、Bold(**太字**)は記事の装飾として必要なため使用OK
- 上記の記号ルールは「本文中の文章表現」に適用する
- 装飾目的のMarkdownまで禁止する趣旨ではない
...
**2026-02-08 作成: inbox「AIっぽくない文章を書く方法」プロンプトをマスタープロンプトとして保存**
